...
function AppContent() {
  const { loading, error, derivedSorted, addTask, updateTask, deleteTask, undoDelete, lastDeleted } =
    useTasksContext();
  ...
  // Undo Snackbar handlers
  const handleCloseUndo = useCallback(() => undoDelete(), [undoDelete]); // no false argument
  const handleUndo = useCallback(() => {
    undoDelete(); // restores lastDeleted
  ...
  const handleAdd = useCallback(
    (payload: Omit<Task, 'id' | 'createdAt'>) => {
      addTask({ ...payload, createdAt: new Date().toISOString() });
